...
*/
class Analytics_MPF_Middle_Metabox_Page 
{
    ...
    public function textareaDisplayMPF( $post ) {
        ...
        $value = get_post_meta( $post->ID, '_analytics_middle_page_meta_key', true );

        wp_nonce_field( basename( __FILE__ ), 'analytics_middle_meta_box_nonce' );

    ?>

         <textarea class="widefat" name="analytics_middle_page_metabox" rows="7"><?php echo $value; ?></textarea>
    ...
    public function saveMetaboxTextArea( $post_id )    {
        ...
        if ( array_key_exists( 'analytics_middle_page_metabox', $_POST ) ) {        
            /**
             *
             * update post meta 
             ...
             *
             */
            $textarea_content = $_POST[ 'analytics_middle_page_metabox' ];
            // $textarea_content = sanitize_text_field( $_POST[ 'analytics_middle_page_metabox' ] );

            update_post_meta( 
                $post_id,                                              // Post ID
                '_analytics_middle_page_meta_key',                              // Meta key            
